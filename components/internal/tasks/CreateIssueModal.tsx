...
export const CreateIssueModal: React.FC<CreateIssueModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
    // Visibility States
    ...
    useEffect(() => {
        if (initialData && isOpen) {
            ...
            if (initialData.type) setIssueType(initialData.type);
        } else if (!isOpen) {
            // Reset when closed
            setTitle('');
            setDescription('');
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col animate-scale-in overflow-hidden">
                            ...
                            <div className="flex items-center justify-between relative z-10">
                                ...
                                <div className="relative">
                                    <button
                                        ...
                                        className="flex items-center gap-2 text-xs font-medium text-slate-700 bg-white border border-slate-200 px-3 py-1.5 rounded-lg hover:border-indigo-300 transition-colors shadow-sm"
                                    >
                                        {issueType} <ChevronDown className="w-3 h-3 text-slate-400" />
                                    </button>

                                    {issueTypeOpen && (
                                        <div className="absolute right-0 top-full mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-xl z-50 py-1">
                                            {['Task', 'Bug', 'User Story', 'Epic'].map(t => (
                                                <button
                                                    key={t}
                                                    onClick={() => { setIssueType(t); setIssueTypeOpen(false); }}
                                                    className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 ${issueType === t ? 'text-indigo-600 font-bold' : 'text-slate-600'}`}
                                                >
                                                    {t}
                                                </button>
                                            ))}
                                        ...
                                        <div className="absolute right-0 top-full mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-xl z-50 py-1">
                                            {['Low', 'Medium', 'High', 'Critical'].map(p => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 ${priority === p ? 'text-indigo-600 font-bold' : 'text-slate-600'}`}
                                                >
                                                    {p}
                                                </button>
                                            ))}
                ...
                <div className="px-6 py-4 border-t border-slate-200 bg-slate-50/50 flex justify-end gap-3 shrink-0">
                    ...
                    <button
                        ...
                        className="px-5 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 shadow-md shadow-indigo-200 transition-all"
                    >
                        Create
                    </button>
                </div>
